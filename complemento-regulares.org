...
***
    - Si \(G\) es 1-regular, entonces \(G\) es una unión disjunta de
      copias de \(K_{2}\).
    - Si \(G=nK_{2}\), entonces \(\overline{G}=O_{n}\)
...
*** 
    ...
    - Si \(r\geq 3\), el resultado se sigue del teorema de los tres
      sumandos.

** Observación

   La única gráfica 2-regular con 8-vértices cuyo complemento es
   convergente es \(C_{4}\cup C_{4}\).

** Gráficas 3-regulares
