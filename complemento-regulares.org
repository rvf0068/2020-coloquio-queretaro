...
** Retracciones especiales
   ...
   - Un octaedro especial en la gráfica \(G\) es una subgráfica \(H\)
     tal que \(H\cong O_{n}\), y tal que un clan de \(H\) es un clan
     de \(G\).
...
** Operaciones en gráficas
   ...
   - Si \(G\) y \(H\) son gráficas, denotamos con \(G\cup H\) a su
     *unión disjunta*.
   - \(G+H\) es la gráfica que se obtiene de \(G\cup H\) añadiendo
     todas las aristas posibles entre un vértice de \(G\) y un vértice
     de \(H\).
...
** Gráficas cúbicas disconexas
   ...
   - Sin embargo, \(\overline{K_{3,3}}+\overline{K_{3,3}}\) *es
     convergente*.
   - Es decir, de las 94 gráficas cúbicas conexas con 12 vértices,
     todo indica que solo una tiene complemento convergente.
...
*** Conjetura
    ...
    Para cada \(n\) par, existe una gráfica cúbica \(G\) con \(n\)
    vértices, tal que \(\overline{G}\) es convergente.
